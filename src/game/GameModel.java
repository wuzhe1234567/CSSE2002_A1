...
public class GameModel {
    ...
    public static final int START_SPAWN_RATE = 2; // 不再随机生成时使用
    ...
    // 注意：Ship 对象不会由 GameModel 内部创建，而是由外部添加到 objects 中
    ...
    /**
     * 更新所有空间对象的状态；当 tick > 0 时，调用 spawnObjects()、levelUp() 与 checkCollisions()。
     */
    ...
    /**
     * 发射子弹：如果存在 Ship，则先从 objects 中移除 Ship，再添加一个 Bullet；否则从默认位置发射。
     */
    ...
    /**
     * 遍历 objects 列表，返回第一个 Ship 对象；如果没有，返回 null。
     */
    public Ship getShip() {
