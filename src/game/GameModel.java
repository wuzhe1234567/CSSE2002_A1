...
public class GameModel {
    ...
    // Ship 对象由外部设置，但为了避免 ship 为 null 导致的异常，
    // 在 updateGame() 中若 ship 为空，则自动创建一个默认的 Ship 对象。
     ...
     * 对于 tick==0，只更新对象；当 tick > 0 时，
     * 若 ship 为空则自动创建默认 ship，并调用 spawnObjects()、levelUp() 与 checkCollisions()。
     */
